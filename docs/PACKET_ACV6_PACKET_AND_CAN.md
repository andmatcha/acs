...
| --- | --- | --- | ---: | --- |
| `PacketMv1` | `M` | `MANUAL` (`flags.bit4..5 == 1`) | 19 byte | `seq`, `current[0..6]`, `control_byte`, 元 `crc16` |
| `PacketIv1` | `I` | `IK` (`flags.bit4..5 == 0`) | 19 byte | `seq`, `current[0]`, `current[1]`, `current[5]`, `current[6]`, `angle[0..2]`, `control_byte`, 元 `crc16` |
| `PacketBv1` | `B` | `KEYBOARD_AUTO` (`flags.bit4..5 == 2`) | 15 byte | `seq`, `angle[0..2]`, `control_byte`, `base_target_mm_j0`, `auto_flags`, 元 `crc16` |

変換時は `AC` の 2 byte header を 1 byte の `M` / `I` / `B` に置換し、不要フィールドを削除して残りの byte を元の順序のまま詰める。末尾の `crc16` は元 `AC v6` の CRC を保持し、縮小後の byte 列に対して再計算しない。
