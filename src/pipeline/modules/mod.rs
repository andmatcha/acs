...
fn build_transform(
    ...
    match config {
        ...
        TransformModuleConfig::Arm9Encode => Ok(Box::new(Arm9EncodeTransform {
            driver: OutputFormat::Arm9.create_driver(),
        })),
...
struct Arm9EncodeTransform {
    driver: Box<dyn crate::output::formats::OutputDriver>,
}

impl MessageTransform for Arm9EncodeTransform {
    fn transform(&mut self, mut message: RouteMessage) -> Result<Vec<RouteMessage>, String> {
        let compact_report: [u8; 8] = message
            .payload
            .as_slice()
            .try_into()
            .map_err(|_| String::from("arm9_encode expects an 8-byte compact report"))?;
